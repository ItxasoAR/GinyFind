...
This document describes the full data processing pipeline used to generate the datasets for training and evaluating the GinyFind model.

The pipeline transforms raw protein structures (PDB files) into residue-level feature datasets suitable for machine learning. It includes chain extraction, homology filtering, dataset splitting, and feature computation.

All steps are fully automated and implemented in the `scripts/` directory.
...
The dataset is automatically constructed from a predefined list of PDB IDs.  
Protein structures are downloaded programmatically using the provided script:
...
**Script:**
`scripts/extract_chains_and_sequences.py`
...
Each residue is converted into a numerical feature vector describing its structural and biochemical environment.
...
**Script:**
`scripts/processing_full_pipeline.py`
...
The entire dataset generation process is reproducible using the provided scripts. No manual intervention is required.
...
* It is provided for reproducibility, model retraining and dataset extension
...
This design enables reliable training and evaluation of machine learning models for ligand-binding site prediction.
